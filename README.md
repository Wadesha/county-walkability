...
- **P1 宏观图接入全部已算县城**：当前 280 县已计算，但宏观图仅接入 168 个（有友好连片者）；其余 112 个已算但未接入下钻，需重跑 `extract_zones.py` 统一接入。
...
5. 宏观图仅接入有友好连片的县城（168/280），其余已算县城暂未下钻。
...
- **已算 280/280** 县城（鲁苏皖豫四省），其中 **168 个**已接入宏观图下钻（含友好连片）；另有 **1 个模拟县城（利津县 370522，own_supplied 且 `simulated:true` 标记）**用于演示，真实数据到达后覆盖即清除。其余 **112 个**已算县城暂未接入宏观下钻（多为无连片友好区，需重跑 `extract_zones.py` 统一接入，见 PRD §9）。
